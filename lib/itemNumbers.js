...
// O número vem como "18  19" quando a mesma linha cobre várias peças iguais
// (quantidade 2, que ocupam dois lugares na caixa). Guardam-se todos os
// lugares, separados por " | " para se lerem bem: "18 | 19".
function normNumber(value) {
  const tokens = normPlain(value).split(/[\s|]+/).filter(Boolean);
  let out = '';
  tokens.forEach((token, i) => {
    if (i === 0) { out = token; return; }
    // A barra só entra entre dois números. A folha tem casos como
    // "294 ou 295" (ainda não se sabe em qual dos lugares está) - aí a
    // palavra fica tal como está, senão lia-se "294 | ou | 295".
    const bothNumbers = /^\d+$/.test(tokens[i - 1]) && /^\d+$/.test(token);
    out += (bothNumbers ? ' | ' : ' ') + token;
  });
  return out;
}

// Só os números, sem separadores - serve para comparar se dois valores têm
// os mesmos lugares, mesmo estando escritos de maneira diferente
// (ex: "66 67 68" e "66 | 67 | 68").
function numberKey(value) {
  return normPlain(value)
    .split(/[\s|]+/)
    .filter(Boolean)
    .join(',');
}
...
function planItemNumbers(parts, { overwrite = false } = {}) {
  ...
  for (const part of ordered) {
    ...
    if (!overwrite && normPlain(part.itemNumber)) {
      ...
      // Se já lá está o número certo mas escrito de outra maneira (ex:
      // "66 67 68" em vez de "66 | 67 | 68"), corrige-se só a apresentação -
      // os lugares são os mesmos, por isso não se está a inventar nada.
      if (hit && numberKey(hit.entry.numero) === numberKey(part.itemNumber)
          && hit.entry.numero !== normPlain(part.itemNumber)) {
        reformatted += 1;
  ...
  return { updates, alreadyHad, notFound, outOfStock, reformatted, byLevel, totalRows: rows.length };
}

module.exports = { planItemNumbers, normRef, normNumber, numberKey };
