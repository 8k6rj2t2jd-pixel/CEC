...
// `Tesseract.recognize()` (a chamada de conveniencia) cria um worker novo -
// carrega o motor LSTM e os dados de idioma - e destroi-o de seguida em
// *cada* leitura, o que demora vários segundos so nesse arranque, ainda
// mais num hosting com pouco CPU. Mantemos um único worker vivo e
// reutilizamo-lo entre leituras: só paga esse custo uma vez.
...
function getWorker() {
  if (!workerPromise) {
    ...
    workerPromise = Tesseract.createWorker('eng', 1, {
      ...
      workerPromise = null; // deixa tentar criar de novo na próxima leitura
      throw err;
    });
  }
...
// O worker só processa um pedido de cada vez - esta fila serializa os
// pedidos concorrentes em vez de os deixar pisar-se uns aos outros.
...
async function readLabel(imagePath) {
  ...
  const run = ocrQueue.then(() => worker.recognize(processedImage));
  ocrQueue = run.then(() => {}, () => {});
